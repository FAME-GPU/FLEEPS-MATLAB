...
fprintf(' enter "help agmg" for more information \n')
   ...
   if (notcpl)
      if (ijb == 0 || ijb >= 2)
        ...
        if (isempty(x0))
           [x,iter,resvec]=dmtlagmg1(A,b,verbose,icg,maxit,tol,ijob );
        else
           if(ijb == 3)
           ...
           [x,iter,resvec]=dmtlagmg1(A,b,verbose,icg,maxit,tol,ijob ,x0);
        end
      else
         dmtlagmg1(A,b,verbose,icg,maxit,tol,ijob );
      end
   else
      if (~preprocessed)
      ...
      if (ijb == 0 || ijb >= 2)
        if (isempty(x0))
           [x,iter,resvec]=zmtlagmg1(A,b,verbose,icg,maxit,tol,ijob );
        else
           if(ijb == 3)
               disp('Warning: the input argument X0 is ignored when IJOB==3')
           elseif(issparse(x0))
           ...
           [x,iter,resvec]=zmtlagmg1(A,b,verbose,icg,maxit,tol,ijob ,x0);
        end
      else
           zmtlagmg1(A,b,verbose,icg,maxit,tol,ijob );
      end
   end
   flag=0;
   if (ijb == 1 || ijb == -1)
