function [amg_CG_cpu_time, x,info,Ai,Bi,BBi,Res,Pro,cl] = amg(fnchand_A_without_R, fnchand_Q_invS,A,b,option,varargin)
...
k = 1; 
x = x0;
r = b - fnchand_A_without_R(x);
nb = max(sqrt(sum(b.^2,1)));
% nb_special = norm(fnchand_Qs_invS(b));
err = zeros(maxIt,2);
...
switch solver
    case 'CG'
        ...
        amg_CG_cpu_time = toc(time_start_amg);
        err = err(1:k,:);
        itStep = k-1;
    case 'VCYCLE'  
        if printlevel >= 1
